...
bot.action('show_commands', async (ctx) => {
    try {
        ...
        const commandList = [
            ...
            '🔹 */منع مستندات* – منع إرسال المستندات',
            '🔹 */تفعيل مستندات* – السماح بإرسال المستندات'
        ];

        const chunkSize = 20; // Adjust this number to fit within Telegram's limits
        const commandChunks = [];

        for (let i = 0; i < commandList.length; i += chunkSize) {
            commandChunks.push(commandList.slice(i, i + chunkSize).join('\n'));
        }
        ...
        // Add a "Back" button after the last chunk
        await ctx.reply('استخدم الزر أدناه للعودة إلى القائمة الرئيسية:', {
            reply_markup: {
