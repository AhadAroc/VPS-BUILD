    ...
    try {
        if (awaitingReplyResponse && tempReplyWord) {
            ...
            try {
                ...
                await ctx.reply('❌ حدث خطأ أثناء حفظ الصورة في قاعدة البيانات. يرجى المحاولة مرة أخرى.');
        ...
        // Reset the awaiting state if we were in that state
...
// Add this function to handle sending replies with media
async function sendReply(ctx, reply) {
    try {
        console.log('Sending reply:', reply);
        
        // Check if the reply has a type field
        if (!reply.type) {
            // If no type field, check for media_type (backward compatibility)
            if (reply.media_type) {
                reply.type = 'media';
        ...
        if (reply.type === 'text') {
            await ctx.reply(reply.reply_text || reply.text);
            return true;
        } else if (reply.type === 'media') {
            const mediaType = reply.media_type;
            const fileId = reply.file_id;
            
            if (!fileId) {
                console.log('No file_id found in media reply:', reply);
                return false;
            }
...
// Add this function to check for automatic replies
async function checkForAutomaticReply(ctx) {
    try {
        const text = ctx.message.text.trim().toLowerCase();
        
        // First check if we're in a group and have a bot ID
        if (ctx.chat.type !== 'private') {
            // For groups, we need to check if there's a specific bot for this group
            const groupId = ctx.chat.id;
            const botId = await database.getBotIdForGroup(groupId);
            
            if (botId) {
                // If we have a bot ID for this group, check for replies in that bot's database
                return await database.getBotReply(botId, text);
            }
        }
        
        // If we're in a private chat or no specific bot for the group,
        // check the main database for replies
        const db = await ensureDatabaseInitialized();
...
// Helper function to get Arabic name for media type
...
async function handleAwaitingReplyResponse(ctx) {
    try {
        ...
        console.log(`Handling text reply for trigger word: ${tempReplyWord}`);
        ...
        console.error('Error handling awaiting reply response:', error);
...
// Add this function to handle awaiting delete reply word
async function handleAwaitingDeleteReplyWord(ctx) {
    try {
        const wordToDelete = ctx.message.text.trim().toLowerCase();
        ...
        console.error('Error handling awaiting delete reply word:', error);
        await ctx.reply('❌ حدث خطأ أثناء حذف الرد. يرجى المحاولة مرة أخرى.');
...
// Add this function to handle awaiting bot name
async function handleAwaitingBotName(ctx) {
    try {
        const newBotName = ctx.message.text.trim();
        
        const db = await ensureDatabaseInitialized();
        await db.collection('bot_settings').updateOne(
            { setting_key: 'bot_name' },
            { $set: { setting_value: newBotName } },
            { upsert: true }
        );
        ...
        console.error('Error handling awaiting bot name:', error);
        await ctx.reply('❌ حدث خطأ أثناء تغيير اسم البوت. يرجى المحاولة مرة أخرى.');
...
// Add this function to load active groups from database
async function loadActiveGroupsFromDatabase() {
    try {
        const db = await ensureDatabaseInitialized();
        const groups = await db.collection('groups').find({ is_active: true }).toArray();
        
        // Clear the current map and populate with database data
        activeGroups.clear();
        for (const group of groups) {
            activeGroups.set(group.group_id, {
...
function setupMediaHandlers(bot) {
    ...
    bot.on('phosssto', async (ctx) => {
        ...
        if (awaitingReplyResponse && tempReplyWord) {
            ...
            console.log('Not awaiting a reply response or no temp word set np');
        }
    });
    ...
    try {
        ...
        if (awaitingReplyResponse) {
            try {
                const db = await ensureDatabaseInitialized();
                await db.collection('replies').insertOne({
                    bot_id: botId, // Include the bot_id
