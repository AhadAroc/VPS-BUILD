...
async function handleBroadcast(ctx) {
    try {
        ...
        if (!(await isDeveloper(ctx, ctx.from.id))) {
            return ctx.reply('⛔ عذرًا، هذا الأمر مخصص للمطورين فقط.');
        }

        const broadcastContent = extractBroadcastContent(ctx);
        if (!broadcastContent) {
            return ctx.reply('❌ لم يتم العثور على محتوى للإذاعة. يرجى إرفاق نص أو وسائط مع الأمر.');
        }

        // Get all active groups from database
        const db = await getDatabaseForBot('test');
        const groups = await db.collection('groups').find({ is_active: true }).toArray();

        if (!groups || groups.length === 0) {
            return ctx.reply('❌ لا توجد مجموعات نشطة للإذاعة.');
        }

        // Send initial status message
        const statusMsg = await ctx.reply(`🔄 جاري الإذاعة إلى ${groups.length} مجموعة...`);
        ...
        for (const group of groups) {
            try {
                switch (broadcastContent.type) {
                    case 'text':
                        await ctx.telegram.sendMessage(group.group_id, broadcastContent.content);
                        break;
                    case 'photo':
                        await ctx.telegram.sendPhoto(group.group_id, broadcastContent.content.file_id, {
                            caption: broadcastContent.content.caption
                        });
                        break;
                    case 'document':
                        await ctx.telegram.sendDocument(group.group_id, broadcastContent.content.file_id, {
                            caption: broadcastContent.content.caption
                        });
                        break;
                    case 'video':
                        await ctx.telegram.sendVideo(group.group_id, broadcastContent.content.file_id, {
                            caption: broadcastContent.content.caption
                        });
                        break;
                }
                ...
                if (successCount % 5 === 0) {
                    await ctx.telegram.editMessageText(
                        ...
                        undefined,
                        `🔄 جاري الإذاعة... ${successCount}/${groups.length} مجموعة`
                    );
                }
                ...
                // If the error is due to the bot being kicked, mark the group as inactive
