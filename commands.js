...
async function isBotAdmin(ctx, userId) {
    try {
        ...
        console.log(`Checking if user ${userIdNum} is a bot admin (اساسي) in chat ${chatIdNum}`);
        
        // Query the database for active bot admins
        ...
        const isAdmin = !!admin;
        console.log(`User ${userIdNum} is${isAdmin ? '' : ' not'} a bot admin (اساسي) in chat ${chatIdNum}`);
        
        return isAdmin;
    } catch (error) {
        console.error('Error checking if user is bot admin (اساسي):', error);
        return false;
    }
}
...
async function promoteToBotAdmin(ctx) {
    try {
        ...
        // Check if the user is already a bot admin - FIXED QUERY
        const existingAdmin = await db.collection('bot_admins').findOne({ 
            user_id: userId,
            chat_id: chatId,
            bot_id: botId,
        ...
        const previousRecord = await db.collection('bot_admins').findOne({
            user_id: userId,
            chat_id: chatId,
            bot_id: botId
        });
...
module.exports = { setupCommands, isAdminOrOwner,showMainMenu,showQuizMenu,getLeaderboard,getDifficultyLevels,updateGroupActivity, getQuestionsForDifficulty,isSecondaryDeveloper,isVIP,chatBroadcastStates,awaitingBroadcastPhoto,updateActiveGroups,handleCommandCallbacks };
